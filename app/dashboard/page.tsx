    ...
    <div className="min-h-screen bg-gray-50/50">
      ...
      <main className="max-w-6xl mx-auto py-10 px-4 space-y-8">
        ...
        <div className="flex items-end justify-between">
          ...
          {/* Only show button if NOT pro */}
          {!isPro && <UpgradeButton />}
        </div>
      </div>

           {/* Card 3: Widget Status (Now Clickable Link) */}
           <a 
             href={`/embed/${user.id}`} 
             target="_blank" 
             rel="noopener noreferrer"
             className="bg-gradient-to-br from-blue-600 to-blue-700 p-6 rounded-2xl shadow-sm flex flex-col justify-between h-32 text-white transition-transform hover:scale-[1.02] cursor-pointer group"
           >
            <div className="flex justify-between items-start">
              <p className="text-blue-100 text-sm font-medium">Public Wall</p>
              <svg className="w-5 h-5 text-blue-200 group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
            </div>
